...
# --- Helper function to remove positional cues ---
def remove_positional_cues(text):
    forbidden = ["top left", "top right", "bottom left", "bottom right"]
...
def eval_loop(
    ...
    for t, R_t in enumerate(trials_Records):
        ...
        # --- Remove positional cues from the initial message ---
        gen_msg = remove_positional_cues(gen_msg)

        # --- MToM pipeline ---
        if spkr_exp_args.model_type != "Human":
            
            # Gather all previous speaker trial records [0..t-1]
            if t > 0:
            ...
            mtom_prompts_path = "/mnt/cimec-storage6/users/samuele.antonelli/mtomicca/MTOM_ICCA/src/args/mtom_prompts_forbidden_history.json"
            ...
            spkr_mtom_prompt = [selected_prompts["spkr_mtom_prompt"]]

            # Filter history before adding it to the prompt
            spkr_history_flat = remove_positional_cues(" ".join(spkr_history_flat))
            lsnr_history_flat = remove_positional_cues(" ".join(lsnr_history_flat))
            ...
            # Use the cleaned initial description
            ...
            # Step 3: refine
            speaker_MToM_addon_prompt = [selected_prompts["spkr_mtom_addon"]]
            ...
            refined_msg = remove_positional_cues(refined_msg)
        ...
        # Save final speaker message as the refined (filtered) message
        ...
        if lsnr_exp_args.model_type == "oracle":
            ...
            # *** Pass the refined message (filtered) to the listener ***
            (
                lsnr_prompt,
                lsnr_trial_prompt,
                lsnr_tgt_img,
                ...
                lsnr_context_imgs,
                tgt_fn,
                msg=refined_msg,  # use refined message here
                records=trials_Records,
            ...
            lsnr_feedback_final = lsnr_model.get_lsnr_feedback(
                lsnr_pred, lsnr_tgt_img, lsnr_trial_imgs, refined_msg  # use refined message here as well
            )
...
def run_test(
    ...
    for i, context_record_fp in tqdm(enumerate(context_records_fps)):
        ...
        dirname_save = os.path.join("outputs_icca_refined", dirname)
...
def main():
    ...
    parser.add_argument("--spkr_intro_texts", type=str, default="args/intro_texts_spkr_forbidden_history.json")
    ...
    parser.add_argument("--img_hosting_site", type=str, default="")
    ...
    print("-------------------------------------------------------" + f"Image Masking Enabled: {lsnr_exp_args.img_mask}")
