...
def eval_loop(
    ...
    for t, R_t in enumerate(trials_Records):
        ...
        if spkr_exp_args.model_type != "Human":
            ...
            if len(spkr_history_all) > 2:
                spkr_history_all = spkr_history_all[-2:]
                spkr_history_flat = list(chain.from_iterable(spkr_history_all))

            # flatten listener lines
            lsnr_history_flat = list(chain.from_iterable(lsnr_history_all))
            if len(lsnr_history_all) > 2:
                lsnr_history_all = lsnr_history_all[-2:]
                lsnr_history_flat = list(chain.from_iterable(lsnr_history_all))

            # Step 2: MToM feedback
            mtom_prompts_path = "/mnt/cimec-storage6/users/samuele.antonelli/mtomicca/MTOM_ICCA/src/args/mtom_prompts_new.json"
